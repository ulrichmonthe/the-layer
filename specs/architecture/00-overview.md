...
**4. Workflow** — Pipeline entities (opportunities, applications, awards, reports), deadlines, tasks, approval gates, and the nudge engine (durable scheduled jobs → email). Push, not pull: the system initiates. Does NOT: auto-send anything externally without an approval record.
